...
func (ws *WebSocket) SetSeshKey(seshKey string) {
	ws.seshKey = seshKey
}

// Subprotocol returns the negotiated protocol for the connection.
func (ws *WebSocket) GetSeshKey() string {
	return ws.seshKey
}

// Subprotocol returns the negotiated protocol for the connection.
func (ws *WebSocket) AddSession(id string, sessionObj interface{}) error {
	if ws.seshKey == "" || ws.sessions[id] != nil {

		return errors.New("Unable to create websocket session id as key is not set or session with id already exists")

	}

	ws.sessions[id] = &sessionObj

	return nil
}

// Subprotocol returns the negotiated protocol for the connection.
func (ws *WebSocket) GetSession(id string) (interface{}, error) {
	if ws.seshKey == "" || ws.sessions[id] == nil {

		return nil, errors.New("Unable to get websocket session id as key is not set or no session with id exists")

	}
	return ws.sessions[id], nil
}
